...
@Component({
    selector: 'app-popover-assign-testing-order',
    templateUrl: './popover-assign-dcf-to-testing-order.component.html',
    styleUrls: ['./popover-assign-dcf-to-testing-order.component.scss'],
})
export class PopoverAssignTONumberComponent implements OnDestroy {
    ...
    @Output()
    readonly assignTestingOrderEmitter: EventEmitter<DcfBinding> = new EventEmitter<DcfBinding>();
    ...
    bindDCFToTestingOrder(): void {
        ...
        if (!this.textareaReasonHasErrors && this.isTextAreaActive) {
            this.assignTestingOrderEmitter.emit(
                new DcfBinding({
