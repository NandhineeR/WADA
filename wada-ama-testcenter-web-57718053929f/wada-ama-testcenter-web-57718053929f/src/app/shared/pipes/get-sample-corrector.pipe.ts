...
@Pipe({
    name: 'getSampleCorrector',
})
export class GetSampleCorrectorPipe implements PipeTransform {
        ...
        return translations$.pipe(
            map(
                (translations: TranslationMap) =>
                    translations[this.translationService.getSampleCorrectorKey(key.toString())]
            )
        );
    }
}
